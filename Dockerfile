...
WORKDIR /usr/src
RUN USER=root cargo new --bin when-works
WORKDIR /usr/src/when-works
...
RUN rm ./target/release/deps/when_works*
RUN cargo build --release

# Strip the binary to reduce size
RUN strip target/release/when-works
...
COPY --from=builder --chown=$APP_USER:$APP_USER /usr/src/when-works/target/release/when-works ${APP}/when-works

# Set proper permissions
RUN chmod +x ${APP}/when-works
